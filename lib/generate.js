  ...
  fs.exists(path, (exists) => {
    if (exists) showMessageLog(`[ERROR] O arquivo ${path} já existe`, 'error');
      ...
      if (arrayActions.length > 0) {
        ...
        buildService(readStream, name, path);
        setTimeout(() => {
  ...
  fs.exists(path, (exists) => {
    if (exists) showMessageLog(`[ERROR] O arquivo ${path} já existe`, 'error');
      ...
      if (arrayActions.length > 0) {
        ...
        buildRequest(readStream, name, path);
        setTimeout(() => {
          arrayActions.forEach(action => {
...
function buildService(readStream, name, path) {
  var templateModel = readStream.toString()
    .replaceAll('$Service', `${camelize(name)}`)
...
function buildRequest(readStream, name, path) {
  var templateRequest = readStream.toString()
    .replaceAll('$Request', `${camelize(name)}`)
